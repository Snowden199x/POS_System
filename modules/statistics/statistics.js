(function () {
  ...
  function renderTrendChart(mode) {
    ...
    trendChart = new Chart(ctx, {
      type: "line",
      data: { labels, datasets: [{ data: values, borderColor: GREEN, backgroundColor: GOLD_T,
        pointBackgroundColor: GREEN, pointRadius: 5, pointHoverRadius: 7,
      ...
      options: {
        ...
        scales: {
          y: { ticks: { callback: (v) => `₱${(v/1000).toFixed(0)}k`,
            font: { family:"Poppins",size:11 }, color:"#5A6B5E" },
  ...
  function buildBarData(mode) {
    if (mode === "weekly") {
      return {
        labels: WEEKLY_DATA.map((w) => { const s=new Date(w.week_start); return `${s.toLocaleString("en",{month:"short"})} ${s.getDate()}`; }),
        values: WEEKLY_DATA.map((w) => parseFloat(w.total_sales)),
      };
    } else {
      return {
        labels: DAILY_DATA.map((d) => { const dt=new Date(d.sale_date); return `${dt.toLocaleString("en",{month:"short"})} ${dt.getDate()}`; }),
  ...
  window.switchBarView = (mode) => renderBarChart(mode);
  ...
  function renderSectionBarChart() {
    ...
    const labels = BAR_DATA.map((d) => { const dt=new Date(d.d); return `${dt.toLocaleString("en",{month:"short"})} ${dt.getDate()}`; });
    const values = BAR_DATA.map((d) => parseInt(d.cnt));
    if (sectionBarChart) sectionBarChart.destroy();
    sectionBarChart = new Chart(ctx, {
      type: "bar",
      data: { labels, datasets: [{ data:values, backgroundColor:GOLD, borderRadius:6, borderSkipped:false, maxBarThickness:29 }] },
      options: {
        responsive: true,
        plugins: { legend:{display:false}, tooltip:{callbacks:{label:(ctx)=>`${ctx.parsed.y} orders`}}, datalabels:false },
        scales: {
          y: { ticks:{stepSize:1,font:{family:"Poppins",size:11},color:"#5A6B5E"}, grid:{color:"rgba(0,0,0,0.05)"}, beginAtZero:true },
          x: { ticks:{font:{family:"Poppins",size:10},color:"#5A6B5E",maxRotation:0,padding:8}, grid:{display:false} },
        },
      },
    });
  }
  ...
  window.openExcelModal = function () {
    ...
    overlay.innerHTML = `
      <div style="background:#F4EFD7;border-radius:24px;padding:28px 32px;width:320px;
        ...
        <select id="ep-year" style="width:100%;margin:6px 0 24px;padding:10px 14px;border-radius:12px;
          border:1.5px solid #DDD3AF;font-family:Poppins,sans-serif;background:white;color:#1C3924;">
          ${yearOptions}
        </select>
  ...
  function buildAllSheetsExcel(data) {
    ...
    // ── SHEET 1: ORDERS ───────────────────────────────────────────────────
    const orderHeaders = ["Order ID","Beeper #","Items","Order Type","Payment",
      "GCash Ref","Subtotal","Discount","Total","Change","Status","Date Ordered","Date Served"];
    const orderRows = (data.orders||[]).map((o)=>[
      o.id, o.beeper_number, o.items_str||"",
      o.order_type==="dine-in"?"Dine In":"Take Out",
      ucfirst(o.payment_method),
      o.gcash_reference||"—",
      parseFloat(o.subtotal||0), parseFloat(o.discount||0),
      parseFloat(o.total||0),   parseFloat(o.change_amount||0),
      ucfirst(o.status),
      o.created_at ? excelDate(o.created_at.split(" ")[0]) : "",
      o.served_at  ? o.served_at.split(" ")[0] : "—",
    ]);
    const ws1 = makeSheet(`TWIST & ROLL POS — Orders ${year}`, orderHeaders, orderRows,
      [10,10,45,12,12,20,12,12,12,12,12,22,22], 10);
    ...
    const ws2 = makeSheet(`TWIST & ROLL POS — Weekly Summary ${year}`, weeklyHeaders, weeklyRows,
      [8,14,14,14,10,10,10,18,20], null);
    XLSX.utils.book_append_sheet(wb, ws2, "Weekly Summary");

    // ── SHEET 3: MONTHLY (all 12 months) ─────────────────────────────────
    // data.monthly always has 12 entries (zeros for empty months) from PHP
    ...
    const monthlyRows = (data.monthly||[]).map((m)=>[
      mNames[(m.mo||1)-1],
      parseInt(m.total_orders||0), parseInt(m.served||0),
      parseInt(m.pending||0),      parseInt(m.voided||0),
      parseFloat(m.total_sales||0), parseFloat(m.total_discounts||0),
      parseFloat(m.avg_order||0),
    ]);
    // Ensure all 12 months present even if PHP missed some
    const monthlyFull = [];
    const byMo = {};
    monthlyRows.forEach((r,i)=>{ byMo[parseInt((data.monthly[i]||{}).mo||i+1)] = r; });
    for (let mo=1; mo<=12; mo++) {
      monthlyFull.push(byMo[mo] || [mNames[mo-1],0,0,0,0,0,0,0]);
    }
    const ws3 = makeSheet(`TWIST & ROLL POS — Monthly Summary ${year}`, monthlyHeaders, monthlyFull,
    ...
    const ws4 = makeSheet(`TWIST & ROLL POS — Annual Summary`, annualHeaders, annualRows,
    ...
    const topRows = (data.top_items||[]).map((item,i)=>[
      i+1, item.name, parseInt(item.total_qty||0), parseFloat(item.total_revenue||0),
    ]);
    const ws5 = makeSheet(
    ...
    XLSX.writeFile(wb, `TwistandRoll_Report_${year}_${dateStr}.xlsx`);
  }
  ...
  function activateChip(chip)   { chip.style.background=GOLD; chip.style.color="#fff"; chip.style.borderColor=GOLD; }
  function deactivateChip(chip) { chip.style.background="#F8F4E4"; chip.style.color=GOLD; chip.style.borderColor=CHIP_BORDER; }
  ...
  window.filterTable = function () {
      ...
      <div style="width:560px;max-width:90vw;background:${MODAL_BG};border-radius:28px;overflow:hidden;box-shadow:0 25px 70px rgba(0,0,0,0.18);font-family:Poppins,sans-serif;">
        ...
        <div style="padding:28px 38px 24px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;gap:18px;">
            <div style="font-size:15px;font-weight:600;color:${GREEN};width:160px;">Date</div>
            <div style="flex:1;display:flex;align-items:center;gap:10px;">
              <input type="date" id="filter-date-from" value="${selectedDateFrom}"
                style="flex:1;height:40px;border-radius:12px;border:1.5px solid ${CHIP_BORDER};padding:0 12px;font-family:Poppins,sans-serif;background:white;color:${GREEN};">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;gap:18px;">
            <div style="font-size:15px;font-weight:600;color:${GREEN};width:160px;">Order type</div>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;gap:18px;">
            <div style="font-size:15px;font-weight:600;color:${GREEN};width:160px;">Payment Method</div>
            <div style="display:flex;gap:8px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:30px;gap:18px;">
            <div style="font-size:15px;font-weight:600;color:${GREEN};width:160px;">Status</div>
            <div style="display:flex;gap:8px;">
    ...
    document.querySelectorAll(".filter-chip").forEach((chip)=>{
      ...
      if (active) activateChip(chip); else deactivateChip(chip);
  ...
  window.applyFilter = function () {
    ...
    getAllTableRows().forEach((row)=>{
      let show=true;
      const dateCell=row.cells[1], orderTypeCell=row.cells[2],
            paymentCell=row.cells[3], statusCell=row.cells[5];
      if (dateCell) {
        const parsed=Date.parse(dateCell.textContent.trim());
        const rowDate=isNaN(parsed)?null:new Date(parsed);
        if (rowDate) {
          const d=rowDate.toISOString().split("T")[0];
          if (from && d<from) show=false;
          if (to   && d>to)   show=false;
        }
      }
      if (selectedOrderType!=="all"     && orderTypeCell && orderTypeCell.textContent.trim().toLowerCase()!==selectedOrderType)     show=false;
      if (selectedPaymentMethod!=="all" && paymentCell   && paymentCell.textContent.trim().toLowerCase()!==selectedPaymentMethod)   show=false;
      if (selectedStatus!=="all"        && statusCell    && statusCell.textContent.trim().toLowerCase()!==selectedStatus)           show=false;
  ...
  function renderTablePagination(page,totalPages,total) {
    ...
    const delta=2; let pageNums=[];
    for (let i=Math.max(1,page-delta); i<=Math.min(totalPages,page+delta); i++) pageNums.push(i);
    let btnHTML="";
    if (pageNums[0]>1) { btnHTML+=`<button class="tpg-btn" data-p="1">1</button>`; if (pageNums[0]>2) btnHTML+=`<span class="tpg-ellipsis">…</span>`; }
    pageNums.forEach((p)=>{ btnHTML+=`<button class="tpg-btn ${p===page?"tpg-btn--active":""}" data-p="${p}">${p}</button>`; });
    if (pageNums[pageNums.length-1]<totalPages) { if (pageNums[pageNums.length-1]<totalPages-1) btnHTML+=`<span class="tpg-ellipsis">…</span>`; btnHTML+=`<button class="tpg-btn" data-p="${totalPages}">${totalPages}</button>`; }
  ...
  function buildAnnualModal() {
    ...
    new Chart(document.getElementById("annualChart"),{
      ...
      options:{
        responsive:true,
        onClick:(e,elements)=>{ if(elements.length>0){const idx=elements[0].index;window.location.href=`?page=statistics&sidebar=1&year=${SELECTED_YEAR}&month=${MONTHLY_DATA[idx].mo}&section=orders`;} },
  ...
  window.openSidebar = function () {
    document.getElementById("stats-sidebar")?.classList.add("stats-sidebar--open");
    const btn=document.getElementById("open-sidebar"); if(btn) btn.style.display="none";
  };
  window.closeSidebar = function () {
    document.getElementById("stats-sidebar")?.classList.remove("stats-sidebar--open");
    const btn=document.getElementById("open-sidebar"); if(btn) btn.style.display="flex";
  };
  ...
  function updateClock() {
    ...
    const dayEl=document.getElementById("current-day"), dateEl=document.getElementById("current-date");
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  ORDER OVERVIEW MODAL — with GCash reference support
  // ══════════════════════════════════════════════════════════════════════
  window.openOrderModal = function (orderData) {
    ...
    const discountRow=discountAmt>0?`
      <div class="summary-row">
        <span>Discount</span>
        <span style="color:#2e6b40;font-weight:600;">−Php ${Number(discountAmt).toLocaleString("en-PH",{minimumFractionDigits:0})}</span>
      </div>`:"";

    // GCash reference row — show original ref and extra ref if present
    const isGcash = (orderData.payment||"").toLowerCase()==="gcash";
    let gcashRefRow = "";
    if (isGcash) {
      if (orderData.gcash_reference) {
        gcashRefRow += `
          <div class="summary-row">
            <span>GCash Ref #</span>
            <span style="font-weight:600;">${orderData.gcash_reference}</span>
          </div>`;
      }
      if (orderData.gcash_reference_extra) {
        gcashRefRow += `
          <div class="summary-row">
            <span>Extra Ref # <span style="font-size:11px;color:#0070C0;">(+₱${Number(orderData.gcash_extra_amount||0).toLocaleString()})</span></span>
            <span style="font-weight:600;color:#0070C0;">${orderData.gcash_reference_extra}</span>
          </div>`;
      }
      if (orderData.refund_amount && parseFloat(orderData.refund_amount) > 0) {
        gcashRefRow += `
          <div class="summary-row">
            <span>Refund</span>
            <span style="font-weight:600;color:#C0392B;">−Php ${Number(orderData.refund_amount).toLocaleString("en-PH",{minimumFractionDigits:0})}</span>
          </div>`;
      }
    }
    ...
    row.addEventListener("click",()=>{
      ...
      try {
        ...
        if (raw) {
          ...
          if (Array.isArray(parsed)) items=parsed.map((i)=>({qty:i.qty,name:i.name,price:i.price}));
        }
      ...
      const gcashRef             = row.getAttribute("data-gcash-ref")||"";
      const gcashRefExtra        = row.getAttribute("data-gcash-ref-extra")||"";
      const gcashExtraAmount     = parseFloat(row.getAttribute("data-gcash-extra-amount")||"0");
      const refundAmount         = parseFloat(row.getAttribute("data-refund-amount")||"0");
      ...
      openOrderModal({
        id: beeperNumber, date: dateText, type: typeText,
        payment: paymentText, status: statusText, items,
        ...
        gcash_reference_extra:  gcashRefExtra,
        gcash_extra_amount:     gcashExtraAmount,
